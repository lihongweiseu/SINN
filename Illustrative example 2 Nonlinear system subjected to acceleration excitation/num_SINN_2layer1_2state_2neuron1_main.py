# %% State integration neural network (SINN) for the numerical case (Sec 3.1)  of Ref:
...
# # Save model
# torch.save(SINN_model.state_dict(), "num_SINN_2layer1_2state_2neuron1_3000_loss5316em2.pt")

# # Load trained model if the users do not want to train the model again
# SINN_model.load_state_dict(torch.load("num_SINN_2layer1_2state_2neuron1_3000_loss5316em2.pt"))
...
i = 29  # select a random integer between 0 and NTest to check the testing performance
plt.plot(t, Test_y_ref[i:i + 1, :].reshape(-1, 1))
plt.plot(t, Test_y_pred1[i:i + 1, :].reshape(-1, 1))
plt.show()

# %% save data
# savemat('num_data_pred1_2layer1_2state_2neuron1_3000.mat', {'Train_y_pred1': Train_y_pred1, 'Test_y_pred1': Test_y_pred1})
